  ...
  describe("WITHOUT shortParticipantNames", () => {
    ...
    describe("Add", () => {
      const contracts = ADDRESSES.map(
        (a, i) => ({ address: a, contractName: `contract${i+1}`})
      );
      ...
      test("potential colliding participants", () => {
        ...
        actors.add(c1); actors.add(c2); actors.add(c3);
      ...
      test("get all participants", () => {
        ...
        for (let i=0; i<5; i++) {
          ...
          expect(participants[i]).toHaveProperty('address', contracts[i].address);
          expect(participants[i]).toHaveProperty('alias', `contract${i+1}_01`);
          expect(participants[i]).toHaveProperty('contractName', `contract${i+1}`);
          expect(participants[i]).toHaveProperty('displayName', `${contracts[i].address}:contract${i+1}`);
        }
    ...
    describe("Add", () => {
      ...
      test("get all participants", () => {
        ...
        for (let i=0; i<5; i++) {
          ...
          expect(participants[i]).toHaveProperty('alias', `contract${i+1}_01`);
          expect(participants[i]).toHaveProperty('contractName', `contract${i+1}`);
          expect(participants[i]).toHaveProperty('displayName', `contract${i+1}_01`);
        }
